...
Created from the version 43 (2013) of the vim plugin: **Align**.

vim page : https://www.vim.org/scripts/script.php?script_id=294
...
```shell
mkdir ~/.vim/pack/vim-align
cd ~/.vim/pack/vim-align
git clone https://github.com/arouene/vim-align.git
```
...
mkdir ~/.vim/pack/vim-align
cd ~/.vim/
git submodule add https://github.com/arouene/vim-align.git pack/vim-align
```

This package is installed in optional packages, it requires to type `:packadd Align` to be loaded before use. That command can also be added in vimrc to load the package at start.

Run the command `:helptags` to generate the doc/tags file.
